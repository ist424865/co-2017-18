...
namespace gr8 {
  ...
  protected:
    ...
    void process_int(int value, int lvl) {
      os() << std::string(lvl, ' ') << "<" << "integer" << ">" << value << "</" << "integer" << ">" << std::endl;
    }
    void process_double(double value, int lvl) {
      os() << std::string(lvl, ' ') << "<" << "double" << ">" << value << "</" << "double" << ">" << std::endl;
    }
    void process_string(std::string *value, int lvl) {
      os() << std::string(lvl, ' ') << "<" << "string" << ">" << *value << "</" << "string" << ">" << std::endl;
    }
    void process_reach(int value, int lvl) {
      switch(value) {
        case 0:
          process_string(new std::string("private"), lvl);
          break;
        case 1:
          process_string(new std::string("public"), lvl);
          break;
        case 2:
          process_string(new std::string("use"), lvl);
          break;
      }
    }
